...
const copyDirContents = require('./fs/copy-dir-contents');
const dirExistsSync = require('./fs/dir-exists-sync');
const dirExists = require('./fs/dir-exists');
const fileExists = require('./fs/file-exists');
 ...
 * @returns {Promise}
 */
// Legacy, fragile method of downloading template from repository.
// Should not be used in any new pieces of the logic if possible
...
async function downloadTemplateFromExamples({ template, name, path: projectPath, isLegacy }) {
  const baseUrl =
    'https://github.com/serverless/examples/tree/add-legacy-templates-from-serverless-repo/';
  const templateUrl = isLegacy ? `${baseUrl}/legacy/${template}` : `${baseUrl}/${template}`;
